 ...
 * SPI has an internal SPI-device counter, for each begin()-call the close()
...
double AS5048A::getRotationInDegrees() {
  ...
  double degrees =
      360.0 * (rotation + AS5048A_MAX_VALUE) / (AS5048A_MAX_VALUE * 2.0);
  return degrees;
}
...
/*
 * Write to a register
 * Takes the 16-bit  address of the target register and the unsigned 16 bit of
 * data to be written to that register Returns the value of the register after
 * the write has been performed. This is read back from the sensor to ensure a
 * sucessful write.
 */
